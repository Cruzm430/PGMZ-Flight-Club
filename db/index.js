...
const syncAndSeed = async() =>{
  ...
  const [GreyCartPlaced, GreyCartUnPlaced, MarkCart, ZachCart, PalakCart] = madeOrders;

  const lineItems = [
    {quantity: 4, size: 10, shoeId: Supreme.id, orderId: GreyCartUnPlaced.id, name: Supreme.name},
    {quantity: 2, size: 9.5, shoeId: NMD.id, orderId: GreyCartPlaced.id, name: NMD.name},
    {quantity: 2, size: 10, shoeId:Supreme.id, orderId: GreyCartPlaced.id, name: Supreme.name},
]
